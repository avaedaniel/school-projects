...
bool my_coin(void *data, void *helper)
{
	...
	return( (brutus->color == cp->color) && (close_enough(brutus->x_position, brutus->y_position,
...
void sweep(void *data)
{
	...
	winnings = delete_new(&brutus->world->coins, my_coin, brutus, 
			free_struct, TEXT);
	if(winnings > 0)
	{
...
{
	iterate_new(world->mascots, sweep);
}
...
{
	return( any_new(world->mascots, is_left_of_flag, NULL));
}
...
void clear_a_list(void *avp, char *name)
{
	...
	d = delete_new(avp, always_true, NULL, free_struct, TEXT);
...
// Ava Daniel edited run_sim
void run_sim(struct Sim *world)
{
	while(game_on(world))
	{
	    sort_new(world->mascots, brutus_Y_order);
	    master_output(world);

	    world->elapsed += DELTA_TIME;
		iterate_new(world->mascots, move_brutus);
	    collect_coins(world);
	}
	sort_new(world->mascots, brutus_score_order);
	final_output(world);
	clear_lists(world);
}
